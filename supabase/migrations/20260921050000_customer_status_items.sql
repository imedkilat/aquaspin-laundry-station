-- Customer clothing/item breakdowns are operational data attached to a transaction.
-- They are intentionally separate from inventory consumption and add-ons.
begin;
set local lock_timeout = '5s';

create table if not exists public.transaction_customer_items (
  ...
  constraint transaction_customer_items_type_check check (
    item_type in ('shorts', 't_shirts', 'pants', 'underwear', 'dresses', 'towels', 'bedsheets', 'jackets', 'other')
  ),
  constraint transaction_customer_items_quantity_check check (quantity > 0),
  constraint transaction_customer_items_custom_name_check check (
    (item_type = 'other' and custom_item_name is not null and char_length(btrim(custom_item_name)) between 1 and 120)
    or (item_type <> 'other' and custom_item_name is null)
  ),
  constraint transaction_customer_items_unique_type unique (transaction_id, item_type)
);

create index if not exists transaction_customer_items_transaction_idx
  on public.transaction_customer_items (transaction_id, item_type);
create index if not exists transaction_customer_items_created_by_idx
  on public.transaction_customer_items (created_by);
create index if not exists transaction_customer_items_updated_by_idx
  on public.transaction_customer_items (updated_by);

alter table public.transaction_customer_items enable row level security;
revoke all on public.transaction_customer_items from public, anon, authenticated;
grant select, insert, update, delete on public.transaction_customer_items to authenticated;

drop policy if exists transaction_customer_items_select on public.transaction_customer_items;
create policy transaction_customer_items_select
  on public.transaction_customer_items
  for select
  to authenticated
  using (
    exists (
...
drop policy if exists transaction_customer_items_insert on public.transaction_customer_items;
create policy transaction_customer_items_insert
  on public.transaction_customer_items
  for insert
  to authenticated
  with check (
...
drop policy if exists transaction_customer_items_update on public.transaction_customer_items;
create policy transaction_customer_items_update
  on public.transaction_customer_items
  for update
  to authenticated
  using (
...
drop policy if exists transaction_customer_items_delete on public.transaction_customer_items;
create policy transaction_customer_items_delete
  on public.transaction_customer_items
  for delete
  to authenticated
  using (
...
begin
  ...
  select order_status into transaction_status
...
drop trigger if exists transaction_customer_items_stamp on public.transaction_customer_items;
create trigger transaction_customer_items_stamp
  before insert or update or delete on public.transaction_customer_items
  for each row execute function private.stamp_transaction_customer_item();
...
-- Completion is blocked in the same audited transition RPC used by the UI.
  ...
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime')
     and not exists (
       ...
       where pubname = 'supabase_realtime'
         ...
         and tablename = 'transaction_customer_items'
     ) then
    alter publication supabase_realtime add table public.transaction_customer_items;
  end if;
end;
$$;
